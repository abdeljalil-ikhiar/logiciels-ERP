...
@AllArgsConstructor
@NoArgsConstructor
@Table(name = "bon_de_reception") // mieux sans accent
public class BonDeReceptionEntity {
  ...
  @JsonIgnoreProperties({"bonDeReceptionEntities", "listAchats"})
  private CommandeAchatsEntity commandeAchatsEntity;

  @OneToMany(mappedBy = "bonDeReceptionEntity",
  ...
  @OneToOne(mappedBy = "bonDeReceptionEntity",
          cascade = CascadeType.ALL,
          orphanRemoval = true)
  @JsonIgnoreProperties({"bonDeReceptionEntity"})
  private BonDeLivraisonFournisseurEntity bonDeLivraisonFournisseurEntity;

  // ✅ Relation avec retours fournisseur
  @OneToMany(mappedBy = "bonDeReception", fetch = FetchType.LAZY)
  @JsonIgnore
  @ToString.Exclude
  private List<RetourProduitEntity> retoursProduit = new ArrayList<>();
}