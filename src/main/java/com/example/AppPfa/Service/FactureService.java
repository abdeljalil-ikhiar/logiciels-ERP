...
public class FactureService implements FactureManager {
    ...
    public FactureEntity addFacture(FactureEntity factureEntity) {
        ...
        // ✅ Associer le bon de livraison
        factureEntity.setBonLivraison(bonLivraison);

        // ✅ Initialiser la liste des lignes si null
        if (factureEntity.getLignesFacture() == null) {
            factureEntity.setLignesFacture(new ArrayList<>());
        }

        // ✅ Créer les lignes de facture à partir des lignes de bon de livraison
        if (factureEntity.getLignesFacture().isEmpty()) {
            for (LigneBonLivraisonEntity ligneBL : bonLivraison.getLignesBonLivraison()) {
                LigneFactureEntity ligneFacture = LigneFactureEntity.builder()
                        ...
                        .totalHT(ligneBL.getTotalHT())
                        .totalTVA(ligneBL.getTotalTVA())
                        .totalTTC(ligneBL.getTotalTTC())
                        .build();
                factureEntity.getLignesFacture().add(ligneFacture);
            }
        } else {
            factureEntity.getLignesFacture().forEach(ligne -> ligne.setFacture(factureEntity));
        }

        // ✅ Calculer les totaux
        calculerTotaux(factureEntity);

        return factureRepository.save(factureEntity);
    }

    /**
     * ✅ AMÉLIORATION: Recalculer la facture à partir du bon de livraison
     * Cette méthode est appelée automatiquement quand le bon de livraison est modifié
     */
    @Transactional
    public FactureEntity recalculerFactureParBonLivraison(Integer bonLivraisonId) {
        ...
        if (factureOpt.isEmpty()) {
            // Pas de facture associée, rien à faire
            return null;
        }
        ...
        // 4️⃣ Recréer les lignes à partir du bon de livraison mis à jour
        double totalHT = 0.0;
        double totalTVA = 0.0;
    ...
    /**
     * ✅ NOUVELLE MÉTHODE: Recalculer la facture par son ID
     */
    @Transactional
